...
import { getContent, transitionContent, getContentIdeas, getActivity } from '@/lib/api';
...
export default function ContentOverviewPage() {
  ...
  const [topIdea, setTopIdea] = useState<any>(null);
  ...
  async function loadContent() {
    try {
      const [c, ideas, actRes] = await Promise.allSettled([
        getContent(id),
        getContentIdeas(id),
        getActivity({ content_item_id: id, limit: 50 }),
      ]);
      if (c.status === 'fulfilled') setContent(c.value);
      if (ideas.status === 'fulfilled') {
        const list = Array.isArray(ideas.value) ? ideas.value : [];
        setTopIdea(list.find((i: any) => i.is_selected) || list[0] || null);
      }
      if (actRes.status === 'fulfilled') {
        const logs = Array.isArray(actRes.value) ? actRes.value : (actRes.value?.data ?? []);
        const createdLog = logs.find((l: any) => l.event_type === 'content_created' && l.metadata?.auto_generated);
        if (createdLog?.metadata?.scores) setAutoScores(createdLog.metadata.scores);
      }
    } catch {}
    setLoading(false);
  }
  ...
  const affiliateHint = notesLines.find((l: string) => l.startsWith('제휴 힌트:'))?.replace('제휴 힌트: ', '') || '';
  const seriesExpansion = notesLines.find((l: string) => l.startsWith('확장 시리즈:'))?.replace('확장 시리즈: ', '') || '';
  const isAutoGenerated = affiliateHint || seriesExpansion || autoScores;
  ...
  return (
    <div style={{ padding: '24px 28px 40px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {isAutoGenerated && (
        <Card>
          <CardHeader><CardTitle>AI 분석</CardTitle></CardHeader>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {autoScores && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                {[
                  { key: 'search',        label: '검색량',    color: '#60a5fa' },
                  { key: 'revenue',       label: '수익성',    color: '#4ade80' },
                  { key: 'internal_link', label: '내부링크',  color: '#a78bfa' },
                  { key: 'difficulty',    label: '난이도역',  color: '#fb923c' },
                ].map(s => {
                  const val = autoScores[s.key] ?? 0;
                  return (
                    <div key={s.key} style={{ padding: '10px 12px', borderRadius: 8, background: `${s.color}08`, border: `1px solid ${s.color}20` }}>
                      <div style={{ fontSize: 10, color: '#5a5870', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        {[1,2,3,4,5].map(dot => (
                          <div key={dot} style={{
                            width: 8, height: 8, borderRadius: '50%',
                            background: dot <= Math.round(val / 20) ? s.color : 'rgba(255,255,255,0.08)',
                          }} />
                        ))}
                        <span style={{ fontSize: 10, color: s.color, marginLeft: 4 }}>{val}</span>
                      </div>
                    </div>
                  );
                })}
            ...
            {/* Idea summary */}
            {topIdea?.result_summary && (
              <div style={{ padding: '12px', borderRadius: 8, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ fontSize: 10, color: '#3a3850', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>아이디어 요약</div>
                <div style={{ fontSize: 12, color: '#9a98a8', lineHeight: 1.6 }}>{topIdea.result_summary}</div>
              </div>
            )}

            {/* Keywords + Titles row */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {topIdea?.seo_keywords?.length > 0 && (
                <div>
                  <div style={{ fontSize: 10, color: '#3a3850', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>SEO 키워드</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                    {topIdea.seo_keywords.map((kw: string, i: number) => (
                      <span key={i} style={{ fontSize: 10, color: '#60a5fa', background: '#60a5fa12', border: '1px solid #60a5fa25', borderRadius: 4, padding: '2px 7px' }}>{kw}</span>
                    ))}
                  </div>
                </div>
              )}
              {topIdea?.suggested_titles?.length > 0 && (
                <div>
                  <div style={{ fontSize: 10, color: '#3a3850', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>추천 제목</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {topIdea.suggested_titles.slice(0, 3).map((t: string, i: number) => (
                      <div key={i} style={{ fontSize: 11, color: '#7a78a8', lineHeight: 1.4 }}>• {t}</div>
                    ))}
                  </div>
                </div>
              )}
            ...
            {/* Hints row */}
            {(affiliateHint || seriesExpansion) && (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {affiliateHint && (
                  <div style={{ fontSize: 11, color: '#c8a96e', background: '#c8a96e12', border: '1px solid #c8a96e30', borderRadius: 6, padding: '4px 10px' }}>
                    💰 {affiliateHint}
                  </div>
                )}
                {seriesExpansion && (
                  <div style={{ fontSize: 11, color: '#a78bfa', background: '#a78bfa12', border: '1px solid #a78bfa30', borderRadius: 6, padding: '4px 10px' }}>
                    📚 {seriesExpansion}
                  </div>
                )}
              </div>
